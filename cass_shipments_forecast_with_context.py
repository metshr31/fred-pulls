#!/usr/bin/env python3
"""
Cass Freight Shipments Forecast Model
Target: FRGSHPUSM649NCIS
...
- Builds true forecast targets for Cass Freight Shipments YoY at horizons 1, 3, 6, and 12 months.
...
TARGET_SERIES = "FRGSHPUSM649NCIS"
...
OUTPUT_XLSX = "cass_freight_shipments_forecast_model_with_context.xlsx"
...
COMPARISON_SERIES = [
    # Keep ATA truck tonnage as a possible predictor/context variable for Cass.
    "TRUCKD11",
    "RRSFS",
    "RSAFS",
    "RSNSR",
...
# Cass Shipments may be closer to shipment counts, orders, sales, inventories,
# and housing/construction activity than ATA tonnage was. These are pulled
# directly by ID; invalid/unavailable IDs are skipped safely by the downloader.
DEMAND_ORDER_SHIPMENT_SERIES = [
    # Manufacturing shipments, orders, inventories, and order books
    "AMTMVS",      # Manufacturers' Value of Shipments: Total Manufacturing
    "AMTMNO",      # Manufacturers' New Orders: Total Manufacturing
    "AMTMTI",      # Manufacturers' Total Inventories: Total Manufacturing
    ...
    "A33SNO",      # Manufacturers' New Orders: Durable Goods, Motor Vehicles and Parts
    "A33SVS",      # Manufacturers' Shipments: Durable Goods, Motor Vehicles and Parts
    "NEWORDER",    # Manufacturers' New Orders: Nondefense Capital Goods Excluding Aircraft

    # Business inventories/sales, wholesale, and retail inventory pressure
    "BUSINV",      # Total Business Inventories
    "ISRATIO",     # Total Business: Inventories to Sales Ratio
    "TOTBUSSMNSA", # Total Business Sales, NSA
    "WHLSLRIRSA",  # Merchant Wholesalers: Inventories to Sales Ratio
    "WHLSLRIMSA",  # Merchant Wholesalers Inventories
    "WHLSLRSA",    # Merchant Wholesalers Sales
    "RETAILIMSA",  # Retailers Inventories

    # Housing/construction flow signals
...
def make_ar_feature_matrix(yoy: pd.DataFrame, mom: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Build autoregressive target-series features available at forecast origin t.

    These features test whether outside indicators add value beyond the target series'
    own momentum. For forecast horizon h, the target is TARGET_YOY[t+h],
...
def build_recommended_forecasts(
    ...
    latest known target month. This is the clean table for practical use.
    """
        ...
        rows.append(
            {
                ...
                "forecast_target_yoy": forecast["forecast_target_yoy"],
...
def build_forecast_experiment(
    ...
    - common_origin: latest month where target series is known, so all model families are comparable
                ...
                latest_forecast_rows.append(
                    {
                        ...
                        "model": baseline_name,
                        "feature_set": "Baseline",
                        "forecast_target_yoy": latest_pred,
                        "n_features": 0,
                        "nonzero_features": 0,
                        "alpha": None,
                        "l1_ratio": None,
                            ...
                            latest_forecast_rows.append(
                                {
                                    ...
                                    "model": display_model_name,
                                    "feature_set": feature_set_name,
                                    "forecast_target_yoy": common_pred,
                                    "n_features": len(selected_cols),
...
# =============================================================================
# Context-informed estimate layer
# =============================================================================

CONTEXT_WARNING = (
    "CONTEXT-INFORMED ESTIMATE — NOT MODEL-CERTIFIED; MAY BE WILDLY OFF. "
    "Use only as a narrative/context layer beside the tested Cass Shipments model-certified forecast."
)

CONTEXT_GROUPS = [
    {
        "group": "Cass Shipments momentum",
        "weight": 0.25,
        "series": [TARGET_SERIES],
        "interpretation": "Direct Cass Freight Shipments momentum. This is the target-series history that the workbook tests as the forecast engine.",
    },
    {
        "group": "ATA truck tonnage context",
        "weight": 0.10,
        "series": ["TRUCKD11"],
        "interpretation": "Weight-side truck freight context. Useful for comparison, but not assumed to predict Cass Shipments unless tests prove it.",
    },
    {
        "group": "Orders and manufacturing shipments",
        "weight": 0.16,
        "series": ["AMTMVS", "AMTMNO", "AMTMUO", "AMDMNO", "AMDMVS", "AMDMUO", "NEWORDER", "A33SNO", "A33SVS"],
        "interpretation": "Manufacturing shipments, new orders, and unfilled orders. These may be closer to shipment activity than broad output indexes.",
    },
    {
        "group": "Inventories and replenishment pressure",
        "weight": 0.14,
        "series": ["BUSINV", "ISRATIO", "AMTMTI", "AMDMTI", "RETAILIRSA", "RETAILIMSA", "WHLSLRIRSA", "WHLSLRIMSA"],
        "interpretation": "Inventory levels and inventory-to-sales ratios. High inventories can suppress shipments; leaner ratios can support replenishment.",
    },
    {
        "group": "Retail and wholesale demand",
        "weight": 0.13,
        "series": ["RSAFS", "RRSFS", "RSNSR", "TOTBUSSMNSA", "WHLSLRSA"],
        "interpretation": "Sales-side goods demand. Dollar sales can overstate physical shipment counts, so this is context unless validated.",
    },
    {
        "group": "Industrial production",
        "weight": 0.11,
        "series": ["INDPRO", "IPMAN", "IPMANSICS", "IPGMFN", "IPGDMFDS", "IPGNMFNS"],
        "interpretation": "Broad goods-producing activity. Supportive when positive, but mostly context rather than forecast proof.",
    },
    {
        "group": "Housing and construction",
        "weight": 0.08,
        "series": ["HOUST", "PERMIT", "COMPUTSA", "TTLCONS", "TLRESCONS", "PNRESCONS", "IPG321S", "MCU321", "IPG327S", "IPG3273S"],
        "interpretation": "Construction-related demand, building products, wood, cement/concrete, and construction spending.",
    },
    {
        "group": "Capacity utilization",
        "weight": 0.06,
        "series": ["TCU", "MCUMFN", "MCUMFND", "MCUMFNN"],
        "interpretation": "How hard factories are running. Helpful for gauging strain/stabilization, not a standalone shipments predictor.",
    },
    {
        "group": "Cass expenditures and cost pressure",
        "weight": 0.07,
        "series": ["FRGEXPUSM649NCIS"],
        "interpretation": "Freight spending pressure. Can rise from rates, fuel, mix, or shipment activity, so it must be interpreted carefully.",
    },
]

SERIES_CONTEXT_NAMES = {
    TARGET_SERIES: "Cass Freight Shipments",
    ...
    "BUSINV": "Total Business Inventories",
    "ISRATIO": "Total Business Inventories-to-Sales Ratio",
    "TOTBUSSMNSA": "Total Business Sales",
    "WHLSLRIRSA": "Merchant Wholesalers Inventories-to-Sales Ratio",
    "WHLSLRIMSA": "Merchant Wholesalers Inventories",
    "WHLSLRSA": "Merchant Wholesalers Sales",
    "RETAILIMSA": "Retailers Inventories",
...
def _latest_nonnull(series: pd.Series) -> tuple[pd.Timestamp | None, float | None]:
    clean = series.dropna()
    if clean.empty:
        return None, None
    return clean.index.max(), float(clean.loc[clean.index.max()])
...
def context_metric_score(series_id: str, latest_yoy: float | None, latest_mom: float | None) -> float:
    """
    Convert latest YoY/MoM into a cautious -1 to +1 context score.

    The score is intentionally simple and transparent. It is not trained, and it is
    not allowed to replace the model-certified forecast.
    """
    if latest_yoy is None and latest_mom is None:
        return np.nan

    yoy = 0.0 if latest_yoy is None or pd.isna(latest_yoy) else latest_yoy
    mom = 0.0 if latest_mom is None or pd.isna(latest_mom) else latest_mom

    # Inventory-to-sales is inverted: lower ratio can imply less inventory overhang.
    if series_id == "RETAILIRSA":
        return _clip((-yoy / 0.04) * 0.70 + (-mom / 0.02) * 0.30, -1.0, 1.0)

    # Cass shipments are especially important because negative shipments can
    # contradict a positive shipments/expenditures story.
    if series_id == "FRGSHPUSM649NCIS":
        return _clip((yoy / 0.06) * 0.75 + (mom / 0.03) * 0.25, -1.0, 1.0)

    if series_id == "FRGEXPUSM649NCIS":
        return _clip((yoy / 0.07) * 0.65 + (mom / 0.04) * 0.35, -1.0, 1.0)

    # Capacity utilization levels are often not percentage-change series in the
    # same sense. Use changes, but keep the threshold conservative.
    if series_id.startswith("MCU") or series_id == "TCU":
        return _clip((yoy / 0.03) * 0.60 + (mom / 0.015) * 0.40, -1.0, 1.0)

    # Default: YoY carries more weight than MoM; MoM handles recent inflection.
    return _clip((yoy / 0.05) * 0.70 + (mom / 0.025) * 0.30, -1.0, 1.0)


def context_tone(score: float | None) -> str:
...
def build_context_metrics(raw: pd.DataFrame, yoy: pd.DataFrame, mom: pd.DataFrame, inventory: pd.DataFrame) -> pd.DataFrame:
    metadata = inventory.set_index("id", drop=False) if "id" in inventory.columns else pd.DataFrame()
    rows = []
    seen = set()

    for group in CONTEXT_GROUPS:
        for series_id in group["series"]:
            if series_id in seen or series_id not in raw.columns:
                continue
            seen.add(series_id)

            raw_date, raw_value = _latest_nonnull(raw[series_id])
            yoy_date, yoy_value = _latest_nonnull(yoy[series_id]) if series_id in yoy.columns else (None, None)
            mom_date, mom_value = _latest_nonnull(mom[series_id]) if series_id in mom.columns else (None, None)
            score = context_metric_score(series_id, yoy_value, mom_value)

            meta = metadata.loc[series_id] if (not metadata.empty and series_id in metadata.index) else pd.Series(dtype=object)
            rows.append(
                {
                    "context_group": group["group"],
                    "group_weight": group["weight"],
                    "series_id": series_id,
                    "series_name": meta.get("title", SERIES_CONTEXT_NAMES.get(series_id, series_id)),
                    "latest_raw_date": raw_date,
                    "latest_raw_value": raw_value,
                    "latest_yoy_date": yoy_date,
                    "latest_yoy": yoy_value,
                    "latest_mom_date": mom_date,
                    "latest_mom": mom_value,
                    "context_score": score,
                    "tone": context_tone(score),
    ...
    if out.empty:
        return out
    return out.sort_values(["context_group", "series_id"]).reset_index(drop=True)


def build_context_summary(context_metrics: pd.DataFrame) -> pd.DataFrame:
    if context_metrics.empty:
        return pd.DataFrame()

    rows = []
    for group_name, group_df in context_metrics.groupby("context_group", sort=False):
        weight = float(group_df["group_weight"].dropna().iloc[0]) if group_df["group_weight"].notna().any() else 0.0
        avg_score = float(group_df["context_score"].dropna().mean()) if group_df["context_score"].notna().any() else np.nan
        latest_points = []
        for _, row in group_df.dropna(subset=["latest_yoy"]).sort_values("latest_yoy", ascending=False).head(3).iterrows():
            latest_points.append(f"{row['series_id']} YoY {row['latest_yoy']:.1%}")
        for _, row in group_df.dropna(subset=["latest_yoy"]).sort_values("latest_yoy", ascending=True).head(2).iterrows():
            if f"{row['series_id']} YoY {row['latest_yoy']:.1%}" not in latest_points:
                latest_points.append(f"{row['series_id']} YoY {row['latest_yoy']:.1%}")

        interpretation = group_df["interpretation"].dropna().iloc[0] if group_df["interpretation"].notna().any() else ""
        rows.append(
            {
                "context_group": group_name,
                "group_weight": weight,
                "avg_context_score": avg_score,
                "tone": context_tone(avg_score),
                "context_points": "; ".join(latest_points),
                "interpretation": interpretation,
            }
        )

    summary = pd.DataFrame(rows)
    weighted = summary.dropna(subset=["avg_context_score"]).copy()
    if not weighted.empty and weighted["group_weight"].sum() > 0:
        total_score = float((weighted["avg_context_score"] * weighted["group_weight"]).sum() / weighted["group_weight"].sum())
    else:
        total_score = np.nan
    ...
    overall = pd.DataFrame(
        [
            {
                "context_group": "OVERALL CONTEXT",
                "group_weight": weighted["group_weight"].sum() if not weighted.empty else np.nan,
                "avg_context_score": total_score,
                "tone": context_tone(total_score),
                "context_points": "Weighted average of context groups above.",
                "interpretation": "Use this only to shade the narrative around the model-certified forecast, not as a validated forecast model.",
            }
        ]
    )
    return pd.concat([overall, summary], ignore_index=True)


def build_context_estimates(recommended_forecasts: pd.DataFrame, context_summary: pd.DataFrame) -> pd.DataFrame:
    if recommended_forecasts.empty:
        return pd.DataFrame()

    overall_score = np.nan
    if not context_summary.empty:
        match = context_summary[context_summary["context_group"] == "OVERALL CONTEXT"]
        if not match.empty:
            overall_score = float(match["avg_context_score"].iloc[0])

    # Context nudge is deliberately capped at +/- 0.75 percentage points.
    # Forecast values are decimal YoY values, so 0.0075 = 0.75 ppt.
    nudge = 0.0 if pd.isna(overall_score) else _clip(overall_score * 0.0075, -0.0075, 0.0075)

    rows = []
    for _, row in recommended_forecasts.iterrows():
        certified = row.get("forecast_target_yoy", np.nan)
        if pd.isna(certified):
            continue

        horizon = int(row.get("horizon"))
        # Wider uncertainty for longer horizons.
        band = {1: 0.030, 3: 0.035, 6: 0.045, 12: 0.060}.get(horizon, 0.050)
        context_estimate = float(certified + nudge)
        ...
        rows.append(
            {
                ...
                "model_recommendation": row.get("recommendation"),
                "model_certified_forecast_yoy": certified,
                "overall_context_score": overall_score,
                "context_nudge": nudge,
                "context_informed_estimate_yoy": context_estimate,
                "illustrative_low_yoy": context_estimate - band,
                "illustrative_high_yoy": context_estimate + band,
...
def build_context_outputs(raw: pd.DataFrame, yoy: pd.DataFrame, mom: pd.DataFrame, inventory: pd.DataFrame, forecast_outputs: dict[str, pd.DataFrame]) -> dict[str, pd.DataFrame]:
    metrics = build_context_metrics(raw, yoy, mom, inventory)
    summary = build_context_summary(metrics)
    estimates = build_context_estimates(forecast_outputs.get("recommended_forecasts", pd.DataFrame()), summary)
    return {
        "context_metrics": metrics,
        "context_summary": summary,
        "context_estimates": estimates,
    }
...
def write_outputs(
    ...
    context_outputs: Optional[dict[str, pd.DataFrame]] = None,
) -> None:
    readme = pd.DataFrame(
        [
            ...
            {"item": "target definition", "value": "Cass Freight Shipments YoY shifted forward by forecast horizon h"},
            {"item": "feature definition", "value": "Candidate YoY lags plus Cass Shipments autoregressive YoY/MoM lags and rolling averages"},
            ...
            {"item": "context layer", "value": "Context Estimates are not model-certified; they are a narrative/judgment layer and may be wildly off."},
        ]
    )
    ...
    with pd.ExcelWriter(output_xlsx, engine="openpyxl") as writer:
        ...
        if context_outputs:
            context_outputs.get("context_estimates", pd.DataFrame()).to_excel(writer, sheet_name="Context Estimates", index=False)
            context_outputs.get("context_summary", pd.DataFrame()).to_excel(writer, sheet_name="Context Summary", index=False)
            context_outputs.get("context_metrics", pd.DataFrame()).to_excel(writer, sheet_name="Context Metrics", index=False)
...
def main() -> None:
    ...
    context_outputs = build_context_outputs(
        raw=raw,
        yoy=yoy,
        mom=mom,
        inventory=primary_inventory,
    ...
    print("\nContext estimates:", flush=True)
    ctx = context_outputs.get("context_estimates", pd.DataFrame())
    if not ctx.empty:
        print(ctx.to_string(index=False), flush=True)
