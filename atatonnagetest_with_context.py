...
CONTEXT_WARNING = (
    ...
    "Use only as a narrative/context layer beside the tested model-certified forecast."
)

CONTEXT_GROUPS = [
    {
        "group": "TRUCKD11 momentum",
        "weight": 0.25,
        "series": [TARGET_SERIES],
        "interpretation": "Direct ATA truck-tonnage momentum. This is the only variable that the workbook supports as the forecast engine.",
    },
    {
    ...
    },
    {
        ...
        "interpretation": "How hard factories are running. Helpful for gauging strain/stabilization, not a standalone tonnage predictor.",
    },
    {
        "group": "Retail demand",
        "weight": 0.15,
        "series": ["RSAFS", "RRSFS", "RSNSR", "RETAILIRSA"],
        "interpretation": "Consumer-goods demand. Dollar sales can overstate physical freight volume, so this is context only.",
    },
    {
        "group": "Cass Freight",
        "weight": 0.15,
        "series": ["FRGSHPUSM649NCIS", "FRGEXPUSM649NCIS"],
        "interpretation": "Shipment and spending backdrop. Negative shipments are a caution flag even when expenditures improve.",
    },
    {
        "group": "Heavy industrial freight",
        "weight": 0.10,
        "series": [
            "IPG331S", "MCU331", "IPG332S", "IPG333S", "MCU333",
            "IPG336S", "MCU336", "IPG3361S", "IPG3363S",
            "IPG3273S", "IPG3331S",
        ],
        "interpretation": "Metals, machinery, transportation equipment, and cement/concrete-type activity. These are freight-relevant heavy-goods signals.",
    },
    {
        "group": "Freight-sensitive weak spots",
        "weight": 0.05,
        "series": ["IPG322S", "IPG3221S", "IPG3222S", "IPG325S", "MCU325", "IPG326S", "MCU326", "IPG321S", "MCU321", "IPG337S"],
        "interpretation": "Packaging/paper, chemicals, plastics/rubber, wood, and furniture. Weakness here argues against a broad freight boom.",
    },
]

SERIES_CONTEXT_NAMES = {
    TARGET_SERIES: "ATA Truck Tonnage Index",
...
def context_metric_score(series_id: str, latest_yoy: float | None, latest_mom: float | None) -> float:
    ...
    # contradict a positive tonnage/expenditure story.
...
def build_context_estimates(recommended_forecasts: pd.DataFrame, context_summary: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in recommended_forecasts.iterrows():
        certified = row.get("forecast_truckd11_yoy", np.nan)
